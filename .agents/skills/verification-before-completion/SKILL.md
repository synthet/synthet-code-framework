...
No completion claims without fresh evidence. Confidence, previous runs, agent reports, and partial checks do not support broad success claims.

## Use with

- [`validate-implementation`](../validate-implementation/SKILL.md) when acceptance criteria must be marked Verified/Failed/Unknown.
...
| Requirements met | Acceptance criteria checked with evidence | Tests pass but requirements were not reread |
...
Treat another agent's success report as a lead, not proof. Inspect the diff and run the relevant checks locally. If independent verification is impossible, mark it as a warning.
