...
# All files under these destination directories are generated by this script; no hand-authored
# files are allowlisted there. Keep source discovery intentionally limited to canonical inputs.
SUBDIRS = [
...
def _diff(src: Path, dst: Path, mode: str) -> list[str]:
    ...
    dst_files = (
        {str(f.relative_to(dst)): f for f in sorted(dst.rglob("*")) if f.is_file()}
        if dst.is_dir()
        else {}
    )
